...
export const NormlizeDrawer: VueComponent2<
  INormalizeModalProps,
  { close: any; change: any }
> = defineComponent({
  ...
  setup(props, context) {
    ...
    function afterVisibleChange(visible: boolean) {
      if (props.afterClose && !visible) requestAnimationFrame(props.afterClose);
    }
    ...
    const [renderScrollbar, windowSize] = useContentRender(
      props,
      () => classes.content,
      headerSelector,
    );
    return () => {
      ...
      return (
        ...
        >
          ...
          {footer && (
            <div class={["ant-modal-footer", classes.footer]}>{footer}</div>
          )}
        </Drawer>
      );
    };
