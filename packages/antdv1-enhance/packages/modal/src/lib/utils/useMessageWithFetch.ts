...
import { Message, Modal, ModalConfirm } from "./core";
  ...
  return Object.assign(
    new Promise<T>(async (resolve, reject) => {
      ...
      try {
        ...
        messageArray(
          "error",
          (castArray(onFailed) as Req[]).concat([
            enforce ? [msg, 150, "error", true] : msg,
          ]),
        );
        reject(e);
      } finally {
...
function getMessage(
  ...
  const [content, timeout = 100, innerType = defaultType, isKeyMessage] =
    castArray(msg) as [string, number?, MessageType?, boolean?];
  if (content) {
...
async function messageArray(type: MessageType = "info", message: Req[]) {
  ...
  for (const msg of list) {
    ...
    if (list.length > 1)
      await new Promise((resolve) => setTimeout(resolve, r[1]));
  }
  return function () {
    close.forEach((h) => h && h());
  };
