...
export function tinycolor(color: any, opts = {}) {
  color = color ? color : "";
...
function flip<K, V>(o: Record<K, V>): Record<V, K> {
  let flipped = {};
  for (let i in o) {
...
tinycolor.colors = {} as typeof tinycolor.names;

for (let i in tinycolor.names) {
  tinycolor.colors[i] = `#${tinycolor.names[i]}`;
}
