...
export function tokenize(fmt: string) {
  const out: ParsedFmt[] = [];
  let start: number = 0;

  let i: number = 0;
  let infmt: boolean = false;
  let fmtparam: string = "",
    fmtflags: string = "",
    fmtwidth: string = "",
    fmtprec: string = "",
...
function doit(tokens: ParsedFmt[], args: any[]): string {
  ...
  for (let i: number = 0; i < tokens.length; ++i) {
    const m: ParsedEntry = tokens[i],
      c: number = m[0].charCodeAt(0);
    ...
    /* flags */
    let flags: string = m[3] || "";
    const alt = flags.includes("#");

    /* position */
    if (m[2]) argidx = parseInt(m[2]) - 1;
    /* %m special case */ else if (c === /*m*/ 109 && !alt) {
      o.push("Success");
      continue;
    }
    ...
    /* grab width */
    let width = 0;
    if (m[4] != null && m[4].length > 0) {
      if (m[4].charAt(0) !== "*") width = parseInt(m[4], 10);
      else if (m[4].length === 1) width = args[idx++];
      else width = args[parseInt(m[4].substr(1), 10) - 1];
    }

    /* grab precision */
    let prec = -1;
    if (m[5] != null && m[5].length > 0) {
      if (m[5].charAt(0) !== "*") prec = parseInt(m[5], 10);
      else if (m[5].length === 1) prec = args[idx++];
      else prec = args[parseInt(m[5].substr(1), 10) - 1];
    }
    ...
    /* grab argument */
    const arg: any = args[argidx];

    /* grab length */
    const len: string = m[6] || "";
    ...
    switch (c) {
      ...
      case /*s*/ 115:
        ...
        if (width > O.length || -width > O.length) {
          if (
            ...
            pad = width - O.length >= 0 ? " ".repeat(width - O.length) : "";
      ...
      case /*c*/ 99:
        switch (typeof arg) {
          case "number": {
            let cc: number = arg;
            if (c == 67 || len.charCodeAt(0) === /*l*/ 108) {
              cc &= 0xffffffff;
      ...
      case /*n*/ 110:
        if (arg) {
          arg.len = 0;
          for (let oo: number = 0; oo < o.length; ++oo) arg.len += o[oo].length;
        }
        continue;
      ...
      case /*J*/ 74:
        O = (alt ? u_inspect : JSON.stringify)(arg);
        break;
      case /*V*/ 86:
        O = arg == null ? "null" : String(arg.valueOf());
        break;
      case /*T*/ 84:
        if (alt) {
          /* from '[object %s]' extract %s */
    ...
    if (isnum == -1) {
      ...
      switch (bytes) {
        case 1:
          Vnum = Vnum & 0xff;
          if (sign && Vnum > 0x7f) Vnum -= 0xff + 1;
          break;
        case 2:
          Vnum = Vnum & 0xffff;
          if (sign && Vnum > 0x7fff) Vnum -= 0xffff + 1;
          break;
        case 4:
          Vnum = sign ? Vnum | 0 : Vnum >>> 0;
          break;
        default:
      ...
      if (len == "L") bytes = 12;
      const isf: boolean = isFinite(Vnum);
      if (!isf) {
        /* Infinity or NaN */
        if (Vnum < 0) O = "-";
        ...
        let sg: string = Vnum < 0 || 1 / Vnum == -Infinity ? "-" : "";
        if (Vnum < 0) Vnum = -Vnum;
        ...
        switch (isnum) {
          ...
          case 4: {
            ...
            O = Vnum.toString(16);
            /* First char 0-9 */
            let ac: number = O.charCodeAt(0);
            if (ac == 48) {
              ac = 2;
              E = -4;
              Vnum *= 16;
              while (O.charCodeAt(ac++) == 48) {
                E -= 4;
                Vnum *= 16;
              }
              O = Vnum.toString(16);
            ...
            const ai: number = O.indexOf(".");
            if (O.indexOf("(") > -1) {
              /* IE exponential form */
              const am = O.match(/\(e(.*)\)/)!;
              const ae: number = am ? +am[1] : 0;
              E += 4 * ae;
              Vnum /= Math.pow(16, ae);
            } else if (ai > 1) {
              E += 4 * (ai - 1);
              Vnum /= Math.pow(16, ai - 1);
            } else if (ai == -1) {
              E += 4 * (O.length - 1);
              Vnum /= Math.pow(16, O.length - 1);
            }
            ...
            if (O.length > 1) {
              if (O.length > prec + 2 && O.charCodeAt(prec + 2) >= 56) {
                const _f: boolean = O.charCodeAt(0) == 102;
  ...
  let result = o.join("");
  for (let i = argidx + 1; i < args.length; i++) {
    result += " " + args[i];
  }
  return result;
}
