...
import { VCProps, VueComponent2 } from "@yuyi919/vue2.7-helper";
import Vue from "vue";
...
import { ThemeGetter, useTheme } from "./provider";
...
>(
  ...
  ...text: TemplateArgs<TProps, ITheme>
) => Component &
...
>(
  ...
  ...text: TemplateArgs<TMixedProps, ITheme>
) => (T extends VueComponent2<any>
  ? T
...
styled.define = function makeUseStyle<TProps extends any>(): {
  css: (...text: TemplateArgs<TProps, ITheme>) => {
...
function makeUseStyle<TProps extends any>(
  ...
  const propsThemeMap = new WeakMap<any, ITheme>();
...
styled.createMixin = function createMixin<TProps extends any = any>(
  ...args: TemplateArgs<TProps, ITheme>
) {
  return (props: TProps, theme?: ITheme) => {
...
function constansts(
  key: string,
): (...text: TemplateArgs<{}, ITheme>) => VueComponent2<{}, any, any> {
